...
class LayerScale(nn.Module):
    def __init__(self, dims, input_size, Layer_scale_init=1.0e-5):
        super().__init__()
        if dims == 1:
            self.layer_scale = nn.Parameter(torch.ones(input_size)*Layer_scale_init, requires_grad=True)
        elif dims == 2:
            self.layer_scale = nn.Parameter(torch.ones(1,input_size)*Layer_scale_init, requires_grad=True)
        elif dims == 3:
            self.layer_scale = nn.Parameter(torch.ones(1,1,input_size)*Layer_scale_init, requires_grad=True)
...
class GCFN(nn.Module):
    def __init__(self, in_channels, dropout_rate, Layer_scale_init=1.0e-5):
        ...
        self.depthwise = nn.Conv1d(in_channels*6, in_channels*6, 3, padding=1, groups=in_channels*6)
...
class EGA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos_k: torch.Tensor):
        ...
        x_down = nn.functional.adaptive_avg_pool1d(input=x, output_size=down_len)
        x = x.permute([0, 2, 1])
        x_down = x_down.permute([0, 2, 1])
        x_down = self.block['self_attn'](x_down, pos_k, None)
        x_down = x_down.permute([0, 2, 1])
        x_downup = nn.functional.upsample(input=x_down, size=x.shape[1])
...
class CLA(nn.Module):
    def __init__(self, in_channels, kernel_size, dropout_rate, Layer_scale_init=1.0e-5):
        ...
        self.GLU = nn.GLU()
        self.dw_conv_1d = nn.Conv1d(in_channels, in_channels, kernel_size, padding='same', groups=in_channels)
