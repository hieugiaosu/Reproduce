...
class FeatureProjector(nn.Module):
    def __init__(self, num_channels: int, in_channels: int, out_channels: int, kernel_size: int, bias: bool):
        super().__init__()
        self.norm = nn.GroupNorm(num_groups=1, num_channels=num_channels, eps=1e-8)
        self.conv1d = nn.Conv1d(
...
class Separator(nn.Module):
    def __init__(self, num_stages: int, relative_positional_encoding: dict, enc_stage: dict, spk_split_stage: dict, simple_fusion:dict, dec_stage: dict):
        ...
        class RelativePositionalEncoding(nn.Module):
            def __init__(self, in_channels: int, num_heads: int, maxlen: int, embed_v=False):
                ...
                self.maxlen = maxlen
                self.pe_k = nn.Embedding(num_embeddings=2*maxlen, embedding_dim=self.embedding_dim)
                self.pe_v = nn.Embedding(num_embeddings=2*maxlen, embedding_dim=self.embedding_dim) if embed_v else None
        ...
        class SpkSplitStage(nn.Module):
            def __init__(self, in_channels: int, num_spks: int):
                ...
                self.linear = nn.Sequential(
                    ...
                    nn.GLU(dim=-2),
                    nn.Conv1d(2*in_channels*num_spks, in_channels*num_spks, kernel_size=1))
        ...
        for _ in range(self.num_stages):
            self.simple_fusion.append(nn.Conv1d(in_channels=simple_fusion['out_channels']*2,out_channels=simple_fusion['out_channels'], kernel_size=1))
    ...
    def forward(self, input: torch.Tensor):
        ...
        for idx in range(self.num_stages):
            ...
            x = nn.functional.upsample(x, skip[idx_en].shape[-1])
